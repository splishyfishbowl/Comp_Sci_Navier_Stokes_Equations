...
As we still had to respect the incompressibility constraints, the solution of the Poisson equation by \textbf{Chorin's Projection method} remained a central numerical challenge.
...
The resulting divergence of the Jacobi method exhibited values of up to $0.0555$. 
These values were too high to reproduce the successful reproduction of convection cells.
Unrealistic behavior occured and can be observed in figure \ref{fig:divjacobi}.
We see, that the spatial distribution of high values of divergence is inhomogeneous.
Around the top ($z=1.0$) and the bottom ($z=0.0$) of our simulated domain values of divergence peaked.
As we were in need to further reduce the divergence as a result of unrealistic simulation results, we tested spectral methods for the solution of the Poission equation.


\subsubsection{Fast Fourier Transform (FFT) Method}

In the final version of our implementation, the Poisson solver was replaced with an FFT-based approach. This method transforms the problem from the spatial domain to the frequency domain, where complex derivatives become simple algebraic divisions.
...
The primary challenge in previous versions of numerical solutions was high divergence caused by the limitations of the Jacobi iterative solver in handling the Poisson equation. By replacing it with the \textbf{Fast Fourier Transform (FFT)} method, the maximum divergence was reduced from $10^{-2}$ to approximately $10^{-15}$. A spatial distribution of the values of divergence is given in figure \ref{fig:divfft}. We can clearly observe uniform low values for this method across the whole domain. 
...
Reaching divergence $\approx 10^{-16}$ signifies that the mass conservation principle is almost perfectly satisfied. This resolves a major bottleneck in CFD codes, providing a robust foundation for the thermal simulation.
...
\end{figure}
We successfuly observed the formation of classical convection cells for the implementation of all described numerical methods, buoyancy term $\beta =50.0$ and viscosity $\nu =0.005$. 
In Figure \ref{fig:convecresults} we see warm plumes (red) rising from the bottom while cold plumes (blue) sink from the top. 
