...
For most PDEs, analytical solutions do not exist, and instead, instead one must use numerical methods to approximate their solutions. However, these 
...
When the first equation is solved numerically, the resulting state is used as the initial condition for the second subproblem. This procedure is
...
The first-order operator splitting solves each operator sequentially over a timestep $\Delta t$. To illustrate this, consider the equation 
\begin{align}
    \partial_t  \bm{u} &= (A+B)\bm{u} \\
\end{align}
Instead of directly solving the full equation, the problem is approximated
...
us to obtain simple solutions for the resulting split-operating problems,
namely
\begin{align}
...
time interval $[t_k, t_{k+1}]=[t_k, t_k + \Delta t]$, rather than continuously solving
the entire time domain. Therefore, the solutions are
...
As mentioned previously, this algorithm relies on the assumption that for
small timesteps $\Delta t$, only small differences remain between 
...
$\mathcal{O}(\Delta t)$, which motivates the
...
\begin{enumerate}
    \item{\textbf{Step 1: Compute the intermediate velocity} \\ Ignoring the
    ...
    $$\nabla \cdot \bm{u}^{k+1} = 0,$$ leads to the Poisson equation for pressure: $$\Delta p = \nabla \cdot \bm{u}^*.$$}
\end{enumerate}
...
are necessary to compute the velocity in our domain.

The physical problem described by Navier-Stokes can be interpreted as
...
motion at one point is influenced by surrounding fluid, etc. These properties are represented by different operators that describe the evolution of the flow.
Our main interest, therefore, is the velocity of the fluid at each grid point in our domain at each timestep. The temporal development of the 
...
volume should be lost or gained during the flow. The pressure, which is
coupled to the velocity field, therefore becomes an additional dynamical
variable that must be solved.
...
timestep. Secondly, pressure is introduced to enforce the
...
\subsection{Operator splitting for the Velocity Equation}
...
\section{Upwind Finite Difference Method}
...
\subsection{Stability of the Operators}
...
\section{Two-Dimensional Upwind Scheme with Operator Splitting}\label{2DUpwind}
...
\begin{tabular}{l c c}
    ...
    \hline
\end{tabular}
\caption[Initial condition modes]{\textbf{Initial condition modes.} Each built-in mode is
...
\section{Chorin's Projection Method}
...
method} \citep{Chorin1968}, which decouples the velocity update from the pressure solve by splitting the computation into three steps at each timestep:
\begin{enumerate}
...
So far we have ignored any forcing in the velocity equation of the Navier-Stokes equation. In the following section, we want to introduce thermal forcing to reproduce the Rayleigh-Bernard
convection cells. Just to name one example, they occur in climatology on different scales and
convective phenomena are important in various proccesses all over the global atmospheric system. In this section, we want to find adequate initial conditions and parameters to reproduce such a convective behavior using our solutions.

\subsection{Physical Model}
The dynamics of Rayleigh--Bénard convection are governed by the Boussinesq approximation, which assumes that density variations are negligible everywhere except in the buoyancy term, where they drive fluid motion. Under this approximation, the system is described by three coupled governing equations. 

The temperature field evolves according to the advection--diffusion equation:
\begin{equation}
\partial_t T + \mathbf{u} \cdot \nabla T = \Delta T .
\end{equation}

The momentum equation including buoyancy is given by:
\begin{equation}
\partial_t \mathbf{u} + (\mathbf{u} \cdot \nabla)\mathbf{u} + \frac{1}{\rho}\nabla p
= \nu \Delta \mathbf{u} + \beta T \mathbf{e}_z .
\end{equation}

In addition, the incompressibility condition must be satisfied:
\begin{equation}
\nabla \cdot \mathbf{u} = 0 .
\end{equation}

The onset of convection is determined by the balance between buoyancy forces, viscous damping, and thermal diffusion. This balance is characterized by the Rayleigh number ($Ra$), a dimensionless parameter defined as the ratio of buoyancy driving to the combined effects of viscous damping and thermal diffusion:
\begin{equation}
Ra = \frac{\text{Buoyancy Force}}{\text{Viscous Damping} \times \text{Thermal Diffusion}} .
\end{equation}

In this system, the buoyancy coefficient (representing the thermal coupling strength) and the kinematic viscosity (representing momentum damping) play key roles in determining the Rayleigh number. If the Rayleigh number remains below a critical threshold, viscous and diffusive effects dominate, and the fluid remains in a purely conductive state. When the Rayleigh number exceeds this critical value, buoyancy overcomes these stabilizing effects, and convection cells begin to form.

\subsection{Initial and Boundary Conditions}
The domain \[\Omega = [0,1]^2\] is periodic in y and wall-bounded in z.
...
\begin{equation}
T(z=0) = 1, \qquad T(z=1) = 0 .
\end{equation}

The velocity field satisfies the no-penetration and free-slip conditions at the boundaries:
\begin{equation}
w = 0, \qquad \frac{\partial v}{\partial z} = 0 .
\end{equation}

\textbf{Initial Conditions}

The initial conditions are designed to trigger the transition from a stable, motionless conductive state to organized convective motion in Rayleigh-Bénard convection. This is achieved by combining a steady-state conductive temperature profile with a small perturbation that breaks the symmetry of the system and induces instability.

\textbf{1. Conductive Base State}

The primary component of the initial temperature field $T_0(y,z)$ is the linear conduction profile. This profile represents the equilibrium state of a purely conductive fluid confined between a hot bottom plate and a cold top plate.
...
This profile ensures that the temperature satisfies the imposed Dirichlet boundary conditions at the plates. At the initial time, the temperature is $T=1$ at the bottom boundary ($z=0$) and $T=0$ at the top boundary ($z=1$).

\textbf{2. Symmetry Breaking via Thermal Perturbation}

A fluid in a perfectly conductive state with no motion will remain static indefinitely unless the equilibrium is disturbed. To initiate the instability that leads to the formation of convective circulation cells, a small thermal perturbation is added to the conductive temperature profile.

The perturbed temperature field is defined as
\begin{equation}
T = (1 - z) + \epsilon \cos\left(\frac{2\pi y}{L_y}\right)\sin\left(\frac{\pi z}{L_z}\right),
\end{equation}

where $\epsilon$ is a small perturbation amplitude (typically in the range $0.01$--$0.05$ in the present implementation).

\textit{Boundary compatibility:}  
The vertical perturbation term $\sin(\pi z / L_z)$ vanishes at the top and bottom plates, ensuring that the imposed temperature boundary conditions remain satisfied.

\textit{Role of the perturbation:}  
This perturbation introduces small local temperature variations, which create density differences in the fluid. These density variations are acted upon by the buoyancy force term $\beta T \mathbf{e}_z$ in the momentum equation, generating the initial vertical motion that eventually develops into convective circulation cells.

\textbf{3. Initial Velocity and Pressure Fields}

The fluid is initially at rest. Therefore, the velocity components are set to zero throughout the domain:
\begin{equation}
v(0,y,z) = 0, \qquad w(0,y,z) = 0 .
\end{equation}

The pressure field is initialized to zero as a reference gauge:
\begin{equation}
p(0,y,z) = 0 .
\end{equation}

As the simulation begins, the buoyancy force generated by the temperature perturbation produces the first vertical motions in the fluid. These motions grow over time and eventually form the closed circulation patterns characteristic of Rayleigh--Bénard convection.



\subsection{Solution Strategy}

The simulation utilises an Explicit Euler scheme for temporal advancement. Although explicit methods are straightforward to implement, they impose a strict constraint on the time step, defined by the Courant-Friedrichs-Lewy (CFL) condition. In our implementation, we selected a grid to ensure that the Courant number remains below 1. This prevents physical information from propagating faster than the numerical scheme, avoiding numerical divergence.

To simulate Rayleigh-Benard Convection under the Boussinesq approximation, we employed Chorin's projection method - to decouple velocity and pressure and enforce divergence-free velocity field - using an explicit Euler scheme to solve the coupled Navier-Stokes and heat equations. Initially, the pressure Poisson equation was addressed using the Jacobi iterative method. While this approach incorporated the horizontal periodic and vertical Neumann boundary conditions, it struggled with convergence; the divergence value was initially high at 1.8, and improved to around $5.55 \times 10^{-2}$ after refinement, with the computational complexity of $O(N^{2})$ per iteration.
Therefore, the main concern was that the convergence values of the solution remain within the allowed limits. For this purpose, individual and overall amounts of divergence values were carefully observed at each step in the development of the solution. The strategy was subsequently upgraded to a spectral FFT-DCT approach, utilising Fast Fourier Transforms for the periodic y-direction and Discrete Cosine Transforms for the vertical z-direction to satisfy the Neumann conditions. This transition transformed the solver's efficiency and precision, reducing the divergence values to $10^{-6}$ and below, while drastically lowering the total computational complexity to $O(NlogN)$. This robust numerical framework successfully generated the characteristic counter-rotating Rayleigh-Benard convectoin rolls, maintaining high fidelity throughout the simulation.